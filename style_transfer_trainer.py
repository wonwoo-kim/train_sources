...
class StyleTransferTrainer:
    ...
    def _build_graph(self):
        ...
        # graph input
        self.y_c = tf.placeholder(tf.float32, shape=self.batch_shape, name='start')
        ...
        self.con1 = tf.constant(255.0, dtype=tf.float32, name='con1')
        self.x = tf.div(self.y_c, self.con1, name='second')
    ...
    def train(self):
        ...
        tf.train.write_graph(self.sess.graph_def, '.', './regression.pb', as_text=False)
        tf.train.write_graph(self.sess.graph_def, '.', './regression.pbtxt', as_text=True)
