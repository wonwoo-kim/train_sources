...
class Transform:
    ...
    def net(self, image):
        ...
        image_p = tf.pad(image,[[0,0],[40,40],[40,40],[0,0]], "CONSTANT", name="first_node")
        ...
        temp0 = tf.nn.tanh(conv_t3, name='tanh')
        t1 = tf.constant(1, dtype=tf.float32)
        t2 = tf.constant(127.5, dtype=tf.float32)
        temp1 = tf.add(temp0, t1, name='addtan1')
        preds = tf.multiply(temp1, t2, name='preds')
        #preds = (tf.nn.tanh(conv_t3) + 1) * (255. / 2)

        return preds

    #def _reflection_padding(self, net):
        #return tf.pad(net,[[0, 0],[40, 40],[40, 40], [0, 0]], "CONSTANT", name='first_node')
    ...
    def _instance_norm(self, net, name=None):
        ...
        self.t3 = tf.constant([1,2])
        mu = tf.reduce_mean(net, self.t3, keepdims=True , name='63mean')
        ans = tf.subtract(net, mu, name='ans')
        ans2 = tf.pow(ans, 2, name='ans2')
        sigma_sq = tf.reduce_mean(ans2,self.t3,keepdims=True, name='63variance')
        ...
        self.t4 = tf.constant(0.001, dtype=tf.float32)
        norm0=tf.subtract(net,mu,name='norm0')
        norm1=tf.add(sigma_sq, self.t4, name='norm1')
        norm2=tf.sqrt(norm1,name='norm2')
        normalized = tf.div(norm0,norm2,name='normalized')
